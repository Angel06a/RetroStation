 ...
 * @param {number} anguloPorOpcion - Ángulo central por ítem en grados.
 * @param {number} totalOpciones - Número total de ítems.
 * @param {Function} corregirHorizontalidad - Función para asegurar la orientación.
...
function calculateAndApplyDimensions(rueda, opciones, initialAngles, anguloPorOpcion, totalOpciones, corregirHorizontalidad) {
    ...
    opciones.forEach((opcion, index) => {
        ...
        // Uso de backticks para legibilidad
        opcion.style.left = `calc(50% + ${xOffset}px)`;
        opcion.style.top = `calc(50% + ${yOffset}px)`;
    });

    // 4. Función de corrección debe ser llamada para asegurar orientación horizontal
...
// Opcional: Exponer la función de métricas para testeo o uso futuro
// window.calculateWheelMetrics = calculateWheelMetrics;